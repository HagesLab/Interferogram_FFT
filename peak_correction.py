...
def subtract_peaks(raw_data, peaks, reduce=[0.9]):
    ...
    reduce : list(floats), optional
        ...
        Reduction factors default to 0.95 (i.e. initial peak will scale down to 95% of
                                          extraneous peak and then be subtracted from
                                          that extraneous peak)
        once all provided factors have been consumed.
        The default is [0.95].
        Alternatively, set a reduction factor to zero to ignore a peak. This is good
        for ignoring false positives.
...
def DEBUG_add_peak(raw_data, existing_peaks, new_peaks):
    largest_peak = [-1,-1,-1,-1]
    for peak_info in existing_peaks:
        if peak_info[3] > largest_peak[3]:
            largest_peak = peak_info
    ...
    left_radius = largest_peak[2] - largest_peak[0]
    right_radius = largest_peak[1] - largest_peak[2]
    for i, peak_info in enumerate(new_peaks):
        new_peak_where = peak_info[0]
        raw_data[new_peak_where-left_radius:new_peak_where+right_radius] += peak_info[1] * largest_peak_values / max(largest_peak_values)
...
if __name__ == "__main__":
    ...
    #     # Where peaks can be randomly inserted, and how large they can be
    #     time_range = (5,40)
    #     mag_range = (3,6)
    #     debug_peaks = [[np.random.uniform(*time_range), 10 ** np.random.uniform(*mag_range)] for i in range(num_tests)]
    #     for new_peak in debug_peaks:
    #         print("DEBUG: Adding random peak of size {} at t={}".format(new_peak[1], new_peak[0]))
    #         new_peak[0] = time_to_index(time_data, new_peak[0])
    #     DEBUG_add_peak(integralTRPL, peaks, debug_peaks)
        
    #     plt.figure(7)
    #     plt.title("DEBUG: Peaks added")
    #     plt.yscale('log')
    #     plt.ylim(np.max(integralTRPL)*TRPLmin_OM,2*np.max(integralTRPL))
    #     plt.plot(time_data, integralTRPL)
        
    #     peaks = find_peaks(integralTRPL, time_data, end_time=40, peak_thr=2, search_radius=5)
